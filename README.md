...
### DEVELOPMENT:
- It dont fit in claude anymore in 1 long initial prompt, so, optimization and creating concise printed lines would be the priority, but, looking at the code, its pretty much there, its only the error handling that seems to have pushed it over the limits. This is what telling it to "improve" scripts generally does, need to stop using that word. Given it implemented a tone of error handling, should probably test.
