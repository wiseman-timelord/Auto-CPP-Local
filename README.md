...
- Solutions have been found to HuggingFace and google, see "NOTATION" section. there will be no APIs in the `.env`, so use a perm `.\data\settings.yaml`, and transfer/remove the env contents. Also move libraries and cache into `.\data\*\`. Create standalone gradio `Configure.Bat` for the pre-launch setup of `.\data\settings.yaml`. Batch files will then be required to be numbered, 1., 2., 3...`requirememnts.txt` would should also be moved to ".\data", and data will obviously become a perm directory.
