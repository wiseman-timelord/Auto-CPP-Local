...
### DEVELOPMENT:
- It dont fit in claude anymore in 1 long initial prompt, with the sections I need to edit, but it gave me a grasp of `if all the files were merged logically, into as few scripts as practical, how much space would each one take up after optimization`, and it turned out to be about the limit of what claude could work with around most of more central scripts. This was my Attempt to create this program using Claude_Sonnet. At the point DeepSeek 2.5 came out, I shelved AutoCpp-Lite it in favor of starting again in other project currently titled `Auto-DeepLitev2`. Parts of this may be merged wih that. This project will either be deleted or revisited, but remains for now as a resource for similar programming interests. End.
