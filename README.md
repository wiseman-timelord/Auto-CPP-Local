...
### DEVELOPMENT:
- It dont fit in claude anymore in 1 long initial prompt, so, optimization and creating concise printed lines would be the priority, but, looking at the code, its pretty much there, its only the error handling that seems to have pushed it over the limits. Given it implemented a ton of error handling, should probably test.
